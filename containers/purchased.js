...
import { getPurchasedWines } from "../actions/winesActions";
...
class Purchased extends Component {
	...
	render() {
		const { purchasedWines } = this.props;

		if (!_.isEmpty(purchasedWines)) {
			return (
				<ScrollView style={styles.container}>
					<View style={{ flex: 1, paddingBottom: 30 }}>
						<SectionList
							sections={purchasedWines}
...
const mapStateToProps = state => ({
	purchasedWines: state.wines.purchasedWines,
});

const mapDispatchToProps = dispatch => ({
	onLoad: token => {
		dispatch(getPurchasedWines(token));
	},
